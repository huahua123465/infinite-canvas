...
function StickyNoteContent({ ctx }: CanvasNodeContentProps) {
    ...
    return (
        <div data-canvas-no-zoom onDoubleClick={(event) => { if (event.target instanceof Element && event.target.closest("[data-canvas-no-drag]")) return; event.stopPropagation(); setEditing(true); }} style={{ position: "relative", height: "100%", width: "100%", display: "flex", flexDirection: "column", background: color, borderRadius: 16, padding: 12, boxSizing: "border-box" }}>
            <div data-canvas-no-drag onMouseDown={(event) => event.stopPropagation()} style={{ display: "flex", gap: 6, justifyContent: "flex-end", marginBottom: 6 }}>
            ...
            {editing ? (
                <textarea data-canvas-no-drag autoFocus value={content} onMouseDown={(event) => event.stopPropagation()} onDoubleClick={(event) => event.stopPropagation()} onChange={(e) => ctx.updateMetadata({ content: e.target.value })} onWheel={(e) => e.stopPropagation()} style={{ flex: 1, resize: "none", border: "none", outline: "none", background: "transparent", color: "#1c1917", fontSize: 15, lineHeight: 1.5 }} />
            ) : (
                <div style={{ flex: 1, whiteSpace: "pre-wrap", overflow: "auto", color: "#1c1917", fontSize: 15, lineHeight: 1.5 }}>{content || "双击正文或点击 ✎ 编辑便利贴"}</div>
            )}
        </div>
    );
}
