...
**方式一:URL 安装。** 构建完成后在「节点插件」管理器的第三方插件地址中填 `/plugins/<name>.js`。构建助手不会自动修改仓库内置插件清单,避免开发插件在其他环境被默认发现或执行。

**方式二:`VITE_DEV_PLUGINS`。** 在 `web/.env.local` 声明(逗号分隔多个),这些插件每次刷新页面都**重新拉取并直接激活**(不缓存、不落库、无开关,仅用于可信本地开发):
