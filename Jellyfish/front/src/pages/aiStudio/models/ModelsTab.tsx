...
import { canvasModelRows, canvasProviderRows, type CanvasBridgeConfig } from '../../../services/canvasBridge'
import {
  MODEL_CATEGORIES,
...
export default function ModelsTab({ canvasConfig }: { canvasConfig?: CanvasBridgeConfig | null }) {
  ...
  const load = async () => {
    setLoading(true)
    if (canvasConfig) {
      setProviders(canvasProviderRows(canvasConfig))
      setModels(canvasModelRows(canvasConfig))
      setLoading(false)
  ...
  /** Refresh provider choices when the model dialog opens, including providers just added in another tab. */
  const refreshProviderOptions = async () => {
    setProviderOptionsLoading(true)
    try {
      const [provRes, supportedRes] = await Promise.all([
        LlmService.listProvidersApiV1LlmProvidersGet({ page: 1, pageSize: 100 }),
        LlmService.listSupportedProvidersApiV1LlmProvidersSupportedGet({}),
      ])
      setProviders(provRes.data?.items ?? [])
      ...
      message.error('供应商列表加载失败，请刷新页面重试')
    } finally {
      setProviderOptionsLoading(false)
    }
  }
  ...
  const getProviderName = (id: string) => {
    ...
    return provider ? (provider.remark ? `${provider.name}（${provider.remark}）` : provider.name) : id
  }
  ...
  const filteredProviderOptions = useMemo(() => {
    if (!selectedFormCategory) return providers.map((p) => ({ label: getProviderName(p.id), value: p.id }))
    return providers
      .filter((provider) => {
        const spec = resolveProviderSpec(provider.name)
        // Keep custom/legacy provider records selectable; the backend remains the final capability validator.
        return !spec || !!spec.supported_categories?.includes(selectedFormCategory)
      })
      .map((p) => ({ label: getProviderName(p.id), value: p.id }))
  ...
  const handleSaveModel = async () => {
    if (canvasConfig) {
      message.info('请回到无限画布配置面板修改模型')
      return
    }
    // New models must use a discovered provider ID; the legacy params field is ignored.
    if (!modelEditing) form.setFieldValue('params', '')
    try {
      const values = await form.validateFields()
      const params: Record<string, unknown> = modelEditing?.params ?? {}
      if (modelEditing) {
        await LlmService.updateModelApiV1LlmModelsModelIdPatch({
          modelId: modelEditing.id,
          requestBody: {
            name: values.discovered_model_name || values.name,
  ...
  /** Discover models for the selected provider so users can choose real provider IDs. */
  const discoverModels = async (providerIdOverride?: string) => {
    const providerId = providerIdOverride ?? (form.getFieldValue('provider_id') as string | undefined)
    if (!providerId) {
    ...
    setDiscovering(true)
    try {
      const result = await LlmService.discoverProviderModelsApiV1LlmProvidersProviderIdDiscoverModelsGet({ providerId })
      setDiscoveredModels((result.data?.models ?? []).map((item) => ({
        name: item.name,
        category: item.category,
        description: item.description ?? '',
      })))
      message.success(`已发现 ${result.data?.models?.length ?? 0} 个模型`)
    } catch {
      message.error('拉取模型失败，请检查 Base URL 和 API Key')
    } finally {
  ...
  useEffect(() => {
    if (!modelModalOpen || modelEditing || providers.length !== 1 || discoveredModels.length > 0) return
    const providerId = providers[0]?.id
    if (!providerId || form.getFieldValue('provider_id')) return
    form.setFieldValue('provider_id', providerId)
    void discoverModels(providerId)
  }, [discoveredModels.length, form, modelEditing, modelModalOpen, providers])
  ...
  return (
    <>
      <div className="flex-shrink-0 px-4 py-2 border-b border-gray-100 bg-white flex flex-wrap items-center justify-between gap-2">
        ...
        <Space wrap>
          <Button type="primary" icon={<PlusOutlined />} onClick={() => openModelModal()} disabled={Boolean(canvasConfig)}>
            添加模型
          </Button>
          <Input
            placeholder="搜索名称/类型"
      ...
      >
        <Form form={form} layout="vertical" className="pt-2">
          <Form.Item name="name" label="模型名称" rules={[{ required: true }]}>
            {modelEditing ? (
              ...
              <Select
                ...
                placeholder="先选择供应商，再拉取模型"
                options={discoveredModels
                  .filter((item) => !selectedFormCategory || item.category === selectedFormCategory)
                  .map((item) => ({
                    label: `${item.name}（${getProviderName(form.getFieldValue('provider_id'))}）`,
                    value: item.name,
                  }))}
                onChange={(name: string) => {
                  const selected = discoveredModels.find((item) => item.name === name)
                  if (selected) form.setFieldValue('category', selected.category)
                }}
              />
            )}
          </Form.Item>
          ...
          {!modelEditing && (
            <Button loading={discovering} onClick={() => void discoverModels()} className="mb-4">
              拉取当前供应商模型
            </Button>
          )}
          <Alert
            type="info"
            showIcon
            message="模型参数由系统按供应商协议维护，不需要手工填写 JSON。"
            className="mb-4"
          />
