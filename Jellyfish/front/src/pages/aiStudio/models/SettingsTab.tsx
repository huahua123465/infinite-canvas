...
import { canvasModelRows, type CanvasBridgeConfig } from '../../../services/canvasBridge'

export default function SettingsTab({ canvasConfig }: { canvasConfig?: CanvasBridgeConfig | null }) {
  ...
  useEffect(() => {
    if (canvasConfig) {
      form.setFieldsValue({
        default_text_model_id: canvasConfig.textModel,
        default_image_model_id: canvasConfig.imageModel,
        default_video_model_id: canvasConfig.videoModel,
      })
    } else if (settings) {
      form.setFieldsValue({
  ...
  }, [canvasConfig, settings, form])

  const handleSaveSettings = async () => {
    if (canvasConfig) {
      message.info('请回到无限画布的配置面板修改模型设置')
      return
    }
    try {
    ...
    <div className="flex-1 overflow-auto p-6 bg-gray-50">
      <Card title="全局默认模型与参数" className="max-w-2xl" loading={loading}>
        ...
        <Form form={form} layout="vertical" onFinish={handleSaveSettings}>
          ...
          <Button type="primary" htmlType="submit" loading={settingsSaving} disabled={Boolean(canvasConfig)}>
            保存设置
          </Button>
        </Form>
      </Card>
