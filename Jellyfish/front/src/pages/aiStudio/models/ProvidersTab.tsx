...
import { canvasProviderRows, type CanvasBridgeConfig } from '../../../services/canvasBridge'
import {
  PROVIDER_STATUS_MAP,
  SORT_OPTIONS,
...
export default function ProvidersTab({ canvasConfig }: { canvasConfig?: CanvasBridgeConfig | null }) {
  ...
  const handleTestConnection = async (provider?: ProviderRead) => {
    if (canvasConfig) {
      message.info('请回到无限画布配置面板测试或修改供应商')
      return
    }
    ...
    try {
      const result = await LlmService.discoverProviderModelsApiV1LlmProvidersProviderIdDiscoverModelsGet({
        providerId: p.id,
      })
      const count = result.data?.models?.length ?? 0
      if (result.data?.source === 'remote') {
        message.success(`连接成功，已读取 ${count} 个模型`)
      } else {
        message.warning('供应商配置已保存，但远程模型接口未响应，当前使用备用模型清单')
      }
    } catch {
  ...
  const providerColumns: TableColumnsType<ProviderRead> = [
    {
      ...
      key: 'name',
      ellipsis: true,
      render: (n: string, record) => <Space>{record.remark ? `${n}（${record.remark}）` : n}</Space>,
    },
    {
      title: 'Base URL',
      dataIndex: 'base_url',
  ...
  return (
    <>
      <div className="flex-shrink-0 px-4 py-2 border-b border-gray-100 bg-white flex flex-wrap items-center justify-between gap-2">
        ...
        <Space wrap>
          <Button type="primary" icon={<PlusOutlined />} onClick={() => openProviderModal()} disabled={Boolean(canvasConfig)}>
            添加供应商
          </Button>
          <Input
            placeholder="搜索名称/描述"
          ...
          {providerList.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {providerList.map((p) => (
                ...
                >
                  <div className="font-medium mb-1">{p.remark ? `${p.name}（${p.remark}）` : p.name}</div>
          ...
          >
            ...
            <div className="p-4 space-y-4">
              <div>
                ...
                <div className="font-medium">{selectedProvider.remark ? `${selectedProvider.name}（${selectedProvider.remark}）` : selectedProvider.name}</div>
              </div>
              <div>
      ...
      >
        <Form form={form} layout="vertical" className="pt-2">
          ...
          <Form.Item name="remark" label="用途备注" extra="用于区分同一供应商的文本、图片、视频等渠道">
            <Input placeholder="例如：图片生成、视频生成、文本提取" maxLength={255} />
          </Form.Item>
