...
@pytest.mark.asyncio
async def test_discover_provider_models_uses_cangyuan_fallback_catalog() -> None:
    """A configured Cangyuan provider remains selectable when its catalog endpoint is unavailable."""
    db, engine = await _build_session()
    async with db:
        await create_provider(
            db,
        ...
        result = await discover_provider_models(db, provider_id="p-cangyuan")
        names = {item.name for item in result.models}
        assert result.source == "fallback"
        assert "nano-banana-pro-1k" in names
        assert any(item.category == ModelCategoryKey.video for item in result.models)
    await engine.dispose()
