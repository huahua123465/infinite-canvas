...
_FALLBACK_MODEL_CATALOG: dict[str, tuple[tuple[str, ModelCategoryKey, str], ...]] = {
    "cangyuan": (
        ("nano-banana-pro-1k", ModelCategoryKey.image, "Nano Banana Pro image generation"),
        ("nano-banana-pro-2k", ModelCategoryKey.image, "Nano Banana Pro image generation"),
        ("nano-banana-pro-4k", ModelCategoryKey.image, "Nano Banana Pro image generation"),
        ("banana-pro-1k", ModelCategoryKey.image, "Banana Pro image generation"),
        ("banana-pro-2k", ModelCategoryKey.image, "Banana Pro image generation"),
        ("banana-pro-4k", ModelCategoryKey.image, "Banana Pro image generation"),
        ("gpt-image-1", ModelCategoryKey.image, "GPT Image generation"),
        ("gpt-image-2", ModelCategoryKey.image, "GPT Image generation"),
        ("gpt-image-1-1k", ModelCategoryKey.image, "GPT Image generation"),
        ("gpt-image-1-2k", ModelCategoryKey.image, "GPT Image generation"),
        ("seedance-2.0", ModelCategoryKey.video, "Seedance video generation"),
        ("seedance-2.0-fast", ModelCategoryKey.video, "Seedance fast video generation"),
    ),
}


def _infer_discovered_category(name: str, provider_key: str) -> ModelCategoryKey | None:
    """Infer a usable capability category from a remote model identifier."""
    value = name.lower()
    if re.search(r"video|seedance|sora|veo|kling|wan|hailuo|runway", value):
        return ModelCategoryKey.video
    if re.search(r"image|imagen|banana|seedream|dall[-_]?e|flux|sdxl|stable[-_]?diffusion", value):
        return ModelCategoryKey.image
    if provider_key == "cangyuan":
        return None
...
async def discover_provider_models(
    ...
    *,
    ...
    """Read a provider model catalog and classify models for user selection."""
    provider = await get_provider(db, provider_id=provider_id)
    bootstrap_all_registries()
    provider_key = resolve_provider_key_from_name(provider.name)
    api_key = (provider.api_key or "").strip()
    base_url = (provider.base_url or "").rstrip("/")
    remote_models: list[ModelDiscoveryItem] = []
    source = "remote"
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                response = await client.get(
                    f"{base_url}/v1/models" if provider_key == "cangyuan" and not base_url.endswith("/v1") else f"{base_url}/models",
                ...
                for item in raw_models:
                    ...
                    if not isinstance(name, str) or not name.strip():
                        continue
                    category = _infer_discovered_category(name, provider_key)
                    if category is not None:
                        remote_models.append(ModelDiscoveryItem(name=name.strip(), category=category))
    ...
    if not remote_models:
        source = "fallback"
        remote_models = [
            ModelDiscoveryItem(name=name, category=category, description=description)
            for name, category, description in _FALLBACK_MODEL_CATALOG.get(provider_key, ())
        ]

    unique = {item.name: item for item in remote_models}
    return ModelDiscoveryRead(provider_id=provider_id, models=list(unique.values()), source=source)
