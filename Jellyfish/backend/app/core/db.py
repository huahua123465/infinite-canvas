    ...
    async with engine.begin() as conn:
        ...
        if conn.dialect.name == "sqlite":
            columns = await conn.run_sync(
                lambda sync_conn: [row[1] for row in sync_conn.exec_driver_sql("PRAGMA table_info(providers)").fetchall()]
            )
            if "remark" not in columns:
                await conn.exec_driver_sql("ALTER TABLE providers ADD COLUMN remark VARCHAR(255) NOT NULL DEFAULT ''")
