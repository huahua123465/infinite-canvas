...
def register_image_model_capability(
    *,
    ...
    if provider == "cangyuan":
        from app.core.integrations.cangyuan.image_capabilities import (
            register_cangyuan_image_capability,
        )

        register_cangyuan_image_capability(model_prefix=model_prefix, capability=capability)
        return
...
def clear_image_model_capability_overrides(*, provider: ProviderKey | None = None) -> None:
    ...
    if provider == "cangyuan":
        from app.core.integrations.cangyuan.image_capabilities import (
            clear_cangyuan_image_capability_overrides,
        )
...
def resolve_image_capability(*, provider: ProviderKey, model: str | None) -> ImageModelCapability:
    ...
    if provider == "cangyuan":
        from app.core.integrations.cangyuan.image_capabilities import (
            resolve_cangyuan_image_capability,
        )

        return resolve_cangyuan_image_capability(model)
