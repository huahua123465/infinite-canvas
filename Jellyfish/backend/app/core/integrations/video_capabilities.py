...
def register_video_model_capability(
    *,
    ...
    if provider == "cangyuan":
        from app.core.integrations.cangyuan.video_capabilities import (
            register_cangyuan_video_capability,
        )

        register_cangyuan_video_capability(model_prefix=model_prefix, capability=capability)
        return
...
def clear_video_model_capability_overrides(*, provider: ProviderKey | None = None) -> None:
    ...
    if provider == "cangyuan":
        from app.core.integrations.cangyuan.video_capabilities import (
            clear_cangyuan_video_capability_overrides,
        )
...
def resolve_video_capability(*, provider: ProviderKey, model: str | None) -> VideoModelCapability:
    ...
    if provider == "cangyuan":
        from app.core.integrations.cangyuan.video_capabilities import (
            resolve_cangyuan_video_capability,
        )

        return resolve_cangyuan_video_capability(model)
