...
class CangyuanImageGenerationTask(AbstractImageGenerationTask):
    """沧元算力图片任务：委托 HTTP 适配器处理同步或异步图片生成。"""

    def __init__(
        self,
        *,
        adapter: CangyuanImageApiAdapter | None = None,
        provider_config: ProviderConfig,
        input_: ImageGenerationInput,
        timeout_s: float = 600.0,
    ) -> None:
        super().__init__(provider_config=provider_config, input_=input_, timeout_s=timeout_s)
        self._adapter = adapter or CangyuanImageApiAdapter()
        self._deferred: ImageGenerationResult | None = None
...
class ImageGenerationTask(BaseTask):
    ...
    def _build_cangyuan_impl(
        *,
        ...
        """构造沧元算力图片任务，保持统一任务分派接口。"""
        return CangyuanImageGenerationTask(
            provider_config=provider_config,
            input_=input_,
            timeout_s=timeout_s,
        )
