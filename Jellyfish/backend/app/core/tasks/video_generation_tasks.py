...
class CangyuanVideoGenerationTask(AbstractVideoGenerationTask):
    """沧元算力视频任务：轮询 queued/in_progress/completed/failed 状态。"""

    def __init__(
        self,
        *,
        adapter: CangyuanVideoApiAdapter | None = None,
        provider_config: ProviderConfig,
        input_: VideoGenerationInput,
        ...
        self._adapter = adapter or CangyuanVideoApiAdapter()
    ...
    async def _poll_and_get_result(self) -> VideoGenerationResult:
        """轮询沧元算力任务，优先使用响应中的成片 URL，缺失时使用 content 端点。"""
        task_id = self._provider_task_id or ""
        if not task_id:
            raise RuntimeError("Cangyuan poll missing provider task id")

        base_url = (self._cfg.base_url or "https://ai.cangyuansuanli.cn").rstrip("/")
        status_val = ""
        video_url: str | None = None
        while True:
            meta = await self._adapter.get_video(
...
class VideoGenerationTask(BaseTask):
    ...
    def _build_cangyuan_impl(
        *,
        ...
        """构造沧元算力视频任务，保持统一任务分派接口。"""
        return CangyuanVideoGenerationTask(
            provider_config=provider_config,
            input_=input_,
            poll_interval_s=poll_interval_s,
            timeout_s=timeout_s,
