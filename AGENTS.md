...
- Script 第三步提示词修正成功后，如果当前章节已经存在视频工作区，必须在不调用文本或视频模型的前提下自动补建缺失的待审核视频草稿；新增草稿只能追加到未占用位置，不得覆盖或移动现有草稿。已有视频结果不能代替或删除对应草稿，草稿重建后必须恢复结果索引；仍未通过生产契约的镜头不得伪造草稿，并须把具体错误写回第三步红色失败状态。
