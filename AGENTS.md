...
- Script 第三步的动态 `videoMotionPrompt` 必须按当前片段总时长输出明确的四段式秒级时间轴，从 0 秒连续覆盖到结束，并固定包含生成规格、资产绑定、叙事目标、起始画面、镜头运动、光线、声音时间轴和稳定约束；最后 1-2 秒保留稳定落点，技能包、内置规则和安全兜底路径必须使用同一格式。
