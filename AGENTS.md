...
- Script 第三步“合成提示词”每个镜头最多只能发起一次付费文本模型请求；模型原始响应必须按镜头连同模型、规则、场景卡和资产上下文指纹保存。JSON 解析、格式、动作或生产契约校验失败后，同一上下文先对已保存响应做本地确定性校正，不得再调用模型做语义重试或 JSON 修复，也不得用本地生成的替代提示词冒充模型结果。校正仍失败时保存错误并继续后续镜头；主动重新合成或上下文变化时不得复用旧响应，确需发起新请求时界面必须明确提示。
