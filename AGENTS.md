...
- Script 画布导出资产时，如果对应待审核视频草稿已经存在，必须同步重算视频草稿的实际参考数组并补齐“资产节点 → 视频草稿”入线；新的自动参考集合必须替换旧的自动人物/场景/道具参考，只保留首尾帧与用户手工加入的参考。普通人物镜头必须分别选择一个最匹配时期的人物参考和一个最匹配地点的场景参考，并且只在当前画面、视觉事实或动作节拍确实出现/使用道具时自动选择最多一个最匹配关键道具；人物和场景严格限制当前章节，道具候选不得因错误或缺失的 `chapterIds` 被提前排除，而应从全系列道具中按当前镜头动作强匹配。道具必须完整命中资产名或至少命中资产名的中文二字片段，资产描述和提示词只能辅助排序，不能单独触发绑定。不能用“每镜已有任意一张参考图”代替场景绑定，也不得把全部道具连接到全部镜头；用户手工加入的参考优先，未进入最终参考数组的资产不得连线。视频提示词正文必须在人物、场景、道具或首帧对应栏目使用与入线素材完全一致的 `@资产名`，场景多角度图必须使用实际 `@场景名-多角度锁定图`；参考预览、画布入线、提示词引用、最终 `@imageN` 顺序和实际请求数组必须保持一致。
