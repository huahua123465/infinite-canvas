...
- Script 画布导出资产时，如果对应待审核视频草稿已经存在，必须同步重算视频草稿的实际参考数组并补齐“资产节点 → 视频草稿”入线；未进入最终参考数组的资产不得连线。视频提示词正文必须在人物、场景、道具或首帧对应栏目使用与入线素材完全一致的 `@资产名`，场景多角度图必须使用实际 `@场景名-多角度锁定图`；参考预览、画布入线、提示词引用、最终 `@imageN` 顺序和实际请求数组必须保持一致。
