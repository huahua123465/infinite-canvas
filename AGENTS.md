...
- 沧元固定分辨率 Seedance 的全能参考请求包含本地视频/音频或超过 4 张本地图时，必须把本地参考图与视频一起临时发布为 HTTPS，避免多张 Base64 图片导致异步转发 JSON 过大或截断；普通全能参考的全部图片必须按顺序放入 `reference_image_urls`，不得因存在参考视频而擅自把第 1 张拆成 `image_url`。只有明确首帧语义时才允许使用主图/首帧字段。
