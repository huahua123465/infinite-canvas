...
- 沧元模型广场 VIDEO 分组的界面提示、生成预检和 API 提交必须共用按 public model 名维护的单一能力表：标准 `seedance-2.0/fast/mini/mini-8s` 为 4 图/3 视频/1 音频，固定档位 `480p/720p/1080p/4k` 及 fast/mini 固定档位为 9 图/3 视频/3 音频，SD5 为 9/3/3 且三类合计不超过 12，Omni Frame 为 5/0/0，Omni V2V 为 0/1/0，Sora 2 为 1/0/0；不得因某次平台内部线路错误擅自降低公开契约上限，也不得静默 `slice` 素材。平台返回更严格限制时应完整展示嵌套 `detail`，作为渠道故障证据交由平台修复。
