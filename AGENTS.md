...
- Script 第三步“合成提示词”每个镜头最多只能发起一次付费文本模型请求；JSON 解析、格式、动作或生产契约校验失败后，只允许对当前模型原始结果做本地确定性校正，不得再调用模型做语义重试或 JSON 修复，也不得用本地生成的替代提示词冒充模型结果。校正仍失败时只保存错误并继续后续镜头；用户再次操作会产生新请求时，界面必须明确提示。
