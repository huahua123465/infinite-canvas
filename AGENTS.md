...
- 沧元视频能力必须至少按渠道、具体请求模型和当前已实证线路组合定义，界面提示、生成预检和 API 提交必须读取同一份有效能力；平台单模型说明中的素材上限不得覆盖真实线路返回的更严格限制，也不得把某一型号的线路限制全局套给同系列其他型号。当前已实证 `seedance-2.0-720p` 带参考视频时会进入 `videos-4`，最多接受 4 张参考图；第 5 张必须在临时发布和付费任务创建前明确阻止，不得静默 `slice`，纯参考图仍按该型号文档保留 9 图能力。
