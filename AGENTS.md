...
- Script 第三步“合成提示词”每个镜头最多只能发起一次付费文本模型请求；JSON 解析、格式、动作或生产契约校验失败后必须立即使用本地确定性兜底，不得再调用模型做语义重试或 JSON 修复。兜底仍失败时只保存本地错误并继续后续镜头；用户再次操作会产生新请求时，界面必须明确提示。
- Script 第三步的动态 `videoMotionPrompt` 必须按当前片段总时长输出明确的四段式秒级时间轴，从 0 秒连续覆盖到结束，并固定包含生成规格、资产绑定、叙事目标、起始画面、镜头运动、光线、声音时间轴和稳定约束；0-3 秒必须在建立场景与站位的同时执行第一步动作链首拍，不得只写建场，最后 1-2 秒保留稳定落点。场景必须在起始画面或首段使用精确 `@资产名`，已绑定且可见的人物和道具必须在实际参与的时间段使用精确 `@资产名`；技能包、内置规则和安全兜底路径必须使用同一格式。
