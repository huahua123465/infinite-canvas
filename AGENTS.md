...
- 沧元 Seedance 使用本地参考视频时，生成链路必须自动把浏览器本地文件临时发布为公网 HTTPS URL，再写入 `reference_videos`；默认临时地址须在一小时内自动过期，任务完成或明确失败后应主动提前删除。发布失败或返回非 HTTPS 地址时不得创建付费视频任务，不得要求用户手工复制命令或回填 URL，也不得把 `blob:`、`data:video` 或本地路径直接提交给沧元。
