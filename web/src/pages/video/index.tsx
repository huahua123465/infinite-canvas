...
export default function VideoPage() {
    ...
    const generate = async () => {
        ...
        setRunning(true);
        const confirmation = await confirmVideoGeneration({ config: snapshot.config, prompt: snapshot.text, references: snapshot.references, videoReferences: snapshot.videoReferences, audioReferences: snapshot.audioReferences });
        if (!confirmation) {
                        ...
                        <div className="mt-6 space-y-5">
                                ...
                                >
                                    {references.map((item, index) => (
                                        <div key={item.id} className="group relative size-20 shrink-0 overflow-hidden rounded-md border border-stone-200 dark:border-stone-800">
                                            ...
                                            {index === 0 ? <span className="absolute bottom-1 left-1 rounded bg-emerald-500 px-1.5 py-0.5 text-[10px] font-semibold text-white">主参考图</span> : null}
                            ...
                            <div className="flex items-center justify-between rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-sm dark:border-stone-800 dark:bg-stone-900 sm:hidden">
                                <span className="truncate text-stone-500 dark:text-stone-400">
                                    {modelOptionLabel(effectiveConfig, model)} · {videoResolutionLabel(effectiveConfig.vquality, model, effectiveConfig)} · {videoSizeLabel(effectiveConfig.size)} · {videoSecondsLabel(effectiveConfig.videoSeconds, model)}
                                </span>
