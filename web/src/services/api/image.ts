...
import { normalizeModelScriptImages, runModelScript } from "@/services/api/model-script-runtime";
...
export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
    ...
    const script = await resolveModelScript("image", selectedModel);
    if (script) {
        const quality = normalizeQuality(config.quality);
...
export async function requestImageQuestion(config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void, options?: RequestOptions) {
    ...
    const requestConfig = resolveModelRequestConfig(config, selectedModel);
    const script = await resolveModelScript("text", selectedModel);
    if (script) {
        try {
            const answer = await runModelScript<string>({ capability: "text", script, config: requestConfig, messages: withSystemMessage(requestConfig, messages), signal: options?.signal, onDelta });
