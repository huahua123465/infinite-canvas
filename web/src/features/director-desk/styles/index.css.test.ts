...
it("renders the viewport aspect ratio picker as a horizontal floating panel", () => {
  ...
  expect(css).toMatch(/\.viewport-aspect-panel\s*\{[\s\S]*?position:\s*absolute;[\s\S]*?bottom:\s*calc\(max\(16px,\s*env\(safe-area-inset-bottom\)\) \+ var\(--viewport-toolbar-height,\s*46px\) \+ 10px\);[\s\S]*?width:\s*340px;[\s\S]*?height:\s*206px;[\s\S]*?border:\s*1px solid rgb\(var\(--border-rgb\) \/ 0\.35\);[\s\S]*?border-radius:\s*22px;[\s\S]*?background:\s*rgb\(var\(--panel-rgb\) \/ 0\.9\);[\s\S]*?backdrop-filter:\s*blur\(32px\);[\s\S]*?-webkit-backdrop-filter:\s*blur\(32px\);/);
