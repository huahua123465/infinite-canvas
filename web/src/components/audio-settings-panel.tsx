    ...
    return (
        <ImageSettingsTheme theme={theme}>
            <div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                ...
                <SettingGroup title={isVolcengine ? "火山音色 / Voice_type" : isVoicebox ? "Voicebox 声音档案" : "OpenAI 声音"} color={theme.node.muted}>
                    {provider.kind === "unsupported" ? (
                        <div className="rounded-lg border px-3 py-2 text-xs leading-5" style={{ borderColor: theme.node.stroke, color: theme.node.muted }}>
                            当前音频模型暂不支持生成，请切换到 OpenAI TTS、火山 OpenSpeech 或 Voicebox。
                        </div>
                    ) : isVolcengine ? (
                        <>
                            <div className="rounded-lg border px-3 py-2 text-xs leading-5" style={{ borderColor: theme.node.stroke, color: theme.node.muted }}>
                ...
                <SettingGroup title="格式" color={theme.node.muted}>
                    {isVoicebox ? (
                        <OptionPill selected theme={theme} onClick={() => onConfigChange("audioFormat", "wav")}>WAV</OptionPill>
                    ) : (
                        <div className="grid grid-cols-3 gap-2.5">
                ...
                <SettingGroup title={isVoicebox ? "Voicebox 表演指令" : "声音指令"} color={theme.node.muted}>
                    <textarea
