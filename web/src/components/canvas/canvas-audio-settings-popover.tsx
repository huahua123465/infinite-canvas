...
export function CanvasAudioSettingsPopover({ config, onConfigChange, buttonClassName, placement = "topLeft" }: CanvasAudioSettingsPopoverProps) {
    ...
    const voiceLabel = provider.kind === "volcengine" ? volcengineVoiceLabel(normalizeVolcengineSpeakerValue(config.audioVoice) || config.audioVoice) : provider.kind === "voicebox" ? "声音档案" : audioVoiceLabel(normalizeAudioVoiceForProvider(config, modelValue));
    const formatLabel = provider.kind === "voicebox" ? "WAV" : audioFormatLabel(config.audioFormat);
