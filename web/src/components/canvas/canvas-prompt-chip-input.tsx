...
import type { CSSProperties, FocusEventHandler, KeyboardEvent, MouseEvent, PointerEvent, PointerEventHandler, WheelEventHandler } from "react";
...
export function CanvasPromptChipInput({ value, references, onChange, onSubmit, className, style, placeholder, onFocus, onBlur, onWheel, onPointerDown }: Props) {
...
function parseTokens(value: string, labels: string[]): Token[] {
    ...
    for (const match of value.matchAll(pattern)) {
        ...
        tokens.push({ type: "reference", label: match[0].replace(/^@/, "") });
