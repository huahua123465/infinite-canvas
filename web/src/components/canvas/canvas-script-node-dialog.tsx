...
type CanvasScriptNodeDialogProps = {
    ...
    onRepairAllProductionProblems: (node: CanvasNodeData) => void;
    onOptimizeFinalReview: (node: CanvasNodeData, issueIds?: string[]) => void;
    onStopFinalReviewOptimization: (node: CanvasNodeData) => void;
    onRepairShot: (node: CanvasNodeData, rowIndex: number) => void;
    onRepairAllShots: (node: CanvasNodeData, rowIndexes: number[]) => void;
                ...
                >
                    ...
                    {shotPlanEditorRowIndex !== null && node.metadata?.storyboardShotPlans?.[String(shotPlanEditorRowIndex)] ? (
                        <ShotPlanEditorModal
                            ...
                            repairError={node.metadata.storyboardShotRepairErrors?.[String(shotPlanEditorRowIndex)]}
...
function ShotsTable({ node, rows, rowIndexes, actionKey, planningStale, productionScope, promptDetails, narrationLocked, narrationIssues, onNarrationLockChange, onCreateChapterNodes, onUpdateCell, onValidateCell, onDeleteRow, onAddRow, onOpenImport, onGenerateShotsFromInputs, onRunFinalReview, onStopFinalReview, onRepairAllProductionProblems, onOptimizeFinalReview, onStopFinalReviewOptimization, onRepairShot, onRepairAllShots, onStopShotRepair, onActiveEpisodeChange, onOpenPrompt, onOpenShotPlan, onGenerateImage, onGenerateVideo, onOpenAssets }: { node: CanvasNodeData; rows: string[][]; rowIndexes: number[]; actionKey?: string | null; planningStale: boolean; productionScope: StoryboardProductionScope; promptDetails: Record<string, StoryboardPromptDetail>; narrationLocked: boolean; narrationIssues: string[]; onNarrationLockChange: (nodeId: string, chapterId: string, locked: boolean) => void; onCreateChapterNodes: (node: CanvasNodeData) => void; onUpdateCell: (rowIndex: number, colIndex: number, value: string) => void; onValidateCell: (rowIndex: number, colIndex: number, value: string) => void; onDeleteRow: (rowIndex: number) => void; onAddRow: () => void; onOpenImport: () => void; onGenerateShotsFromInputs: (node: CanvasNodeData) => void; onRunFinalReview: (node: CanvasNodeData) => void; onStopFinalReview: (node: CanvasNodeData) => void; onRepairAllProductionProblems: (node: CanvasNodeData) => void; onOptimizeFinalReview: (node: CanvasNodeData, issueIds?: string[]) => void; onStopFinalReviewOptimization: (node: CanvasNodeData) => void; onRepairShot: (node: CanvasNodeData, rowIndex: number) => void; onRepairAllShots: (node: CanvasNodeData, rowIndexes: number[]) => void; onStopShotRepair: (node: CanvasNodeData) => void; onActiveEpisodeChange: (nodeId: string, episodeId: string) => void; onOpenPrompt: (rowIndex: number) => void; onOpenShotPlan: (rowIndex: number) => void; onGenerateImage: (node: CanvasNodeData, rowIndex: number) => void; onGenerateVideo: (node: CanvasNodeData, rowIndex: number) => void; onOpenAssets: () => void }) {
    ...
    const locateShot = (rowIndex: number) => {
        const chapter = chapters.find((item) => item.shotIndexes.includes(rowIndex));
        if (chapter && chapter.id !== activeEpisode?.id) onActiveEpisodeChange(node.id, chapter.id);
        setPendingShotIndex(rowIndex);
    };
    useEffect(() => {
        if (pendingShotIndex === null || !rowIndexes.includes(pendingShotIndex)) return;
        const target = document.querySelector<HTMLElement>(`[data-storyboard-shot-index="${pendingShotIndex}"]`);
        if (!target) return;
        target.scrollIntoView({ behavior: "smooth", block: "center" });
        target.focus({ preventScroll: true });
        setFocusedShotIndex(pendingShotIndex);
            ...
            <div className="thin-scrollbar min-h-0 flex-1 overflow-auto bg-[#0d0f0e] px-8 py-6">
                <div className="mx-auto flex max-w-[1500px] flex-col gap-4">
                    {displayRows.map(({ row, rowIndex }) => {
                        ...
                        const repairError = node.metadata?.storyboardShotRepairErrors?.[String(rowIndex)];
                        return (
                            <article key={rowIndex} data-storyboard-shot-index={rowIndex} tabIndex={-1} className={`overflow-hidden rounded-xl border bg-[#171a18] shadow-[0_14px_40px_rgba(0,0,0,.18)] outline-none transition-all duration-500 ${focusedShotIndex === rowIndex ? "border-emerald-300/70 ring-2 ring-emerald-300/25" : "border-white/10"}`}>
                                ...
                                <div className="grid grid-cols-[minmax(0,1.45fr)_minmax(360px,.55fr)]">
                                    ...
                                    <aside className="flex min-w-0 flex-col gap-4 bg-white/[0.015] p-5">
                                        ...
                                        {qualityError || repairError ? (
                                            <div className="rounded-lg border border-amber-400/20 bg-amber-500/8 p-3 text-xs leading-5 text-amber-100">
                                                <div className="font-semibold">{repairError ? `最近修正失败 · ${storyboardShotRepairStageLabel(repairError.stage)}` : "生产契约未通过"}</div>
                                                <div className="mt-1 text-amber-100/75">{repairError?.message || qualityError}</div>
                                                {repairError && qualityError && repairError.stage !== "contract" ? <div className="mt-2 border-t border-amber-300/10 pt-2 text-amber-100/55">当前生产合同：{qualityError}</div> : null}
...
function StoryboardFinalReviewPanel({ node, actionKey, contextKey, onRun, onStop, onRepairAll, onOptimize, onStopOptimization, onLocateShot }: { node: CanvasNodeData; actionKey?: string | null; contextKey: string; onRun: () => void; onStop: () => void; onRepairAll: () => void; onOptimize: (issueIds?: string[]) => void; onStopOptimization: () => void; onLocateShot: (rowIndex: number) => void }) {
    ...
    const optimizing = actionKey === "final-review-optimize";
    const interrupted = !running && !optimizing && (progress?.status === "analyzing" || progress?.status === "reviewing");
    const issues = review?.issues || [];
    const optimizableIssueIds = Array.from(new Set([
        ...issues.filter((issue) => issue.severity !== "P0" && issue.shotIndexes.length).map((issue) => issue.id),
        ...(review?.deterministicFindings || []).filter((issue) => issue.severity !== "P0" && issue.shotIndexes.length).map((issue) => issue.id),
    ]));
    return (
        <section className="shrink-0 border-b border-emerald-400/10 bg-[#121715] px-8 py-3 text-xs text-[#c8d1cc]">
            <div className="flex items-start gap-5">
                            ...
                            {issues.length ? (
                                <div className="mt-3 grid gap-2 lg:grid-cols-3">
                                    {(["P0", "P1", "P2"] as const).map((severity) => {
                                        ...
                                        return <div key={severity} className="rounded-md border border-white/8 bg-black/15 p-3"><div className={`font-semibold ${severity === "P0" ? "text-red-200" : severity === "P1" ? "text-amber-200" : "text-cyan-100"}`}>{severity} · {scoped.length} 项</div><div className="mt-2 space-y-3">{scoped.slice(0, 3).map((issue) => <div key={issue.id} className="border-t border-white/5 pt-2 first:border-0 first:pt-0"><div className="font-semibold text-white">{issue.title}</div><div className="mt-0.5 line-clamp-2 leading-5 text-[#9fa9a3]" title={issue.description}>{issue.description}</div>{issue.shotIndexes.length || issue.beatIds.length ? <div className="mt-1 text-[11px] text-[#78827c]">{issue.shotIndexes.length ? `镜头 ${issue.shotIndexes.map((index) => index + 1).join("、")}` : ""}{issue.shotIndexes.length && issue.beatIds.length ? " · " : ""}{issue.beatIds.length ? `事实 ${issue.beatIds.join("、")}` : ""}</div> : null}<div className="mt-1 line-clamp-2 text-emerald-100/80" title={issue.suggestion}>建议：{issue.suggestion}</div>{issue.shotIndexes.length ? <Button type="link" size="small" className="!-ml-2 !mt-1 !h-6 !px-2 !text-emerald-200" onClick={() => onLocateShot(issue.shotIndexes[0])}>定位镜头 {issue.shotIndexes[0] + 1}</Button> : <div className="mt-2 text-[11px] font-semibold text-amber-200/80">需人工调整 · 不支持自动优化</div>}</div>)}{scoped.length > 3 ? <div className="text-[11px] text-[#77817b]">另有 {scoped.length - 3} 项，重新终审后按优先级继续处理</div> : null}</div></div>;
                                    })}
                                </div>
                            ) : null}
                        </div>
                ...
                <div className="flex shrink-0 flex-col items-end gap-2">
                    {review && !review.productionGate.passed ? <Button danger type="primary" size="small" disabled={Boolean(actionKey)} icon={<Sparkles className="size-3.5" />} onClick={onRepairAll}>修复全部生产问题</Button> : null}
                    {review?.productionGate.passed && optimizableIssueIds.length ? <Button type="primary" size="small" danger={optimizing} disabled={!optimizing && Boolean(actionKey)} icon={optimizing ? <Square className="size-3.5" /> : <Sparkles className="size-3.5" />} onClick={optimizing ? onStopOptimization : () => onOptimize()}>{optimizing ? "停止审美优化" : `按终审建议优化 ${optimizableIssueIds.length} 项`}</Button> : null}
                    {review?.productionGate.passed && issues.some((issue) => issue.severity !== "P0" && !issue.shotIndexes.length) && !optimizableIssueIds.length ? <div className="max-w-44 text-right text-[10px] leading-4 text-amber-200/75">当前问题需人工调整，不支持自动优化</div> : null}
...
function storyboardShotRepairStageLabel(stage: "transport" | "json" | "contract") {
    return stage === "transport" ? "上游请求" : stage === "json" ? "JSON 解析" : "生产合同";
}

function ShotPlanEditorModal({ row, plan, repairError, onClose, onSave }: { row: string[]; plan: StoryboardShotPlan; repairError?: { stage: "transport" | "json" | "contract"; message: string }; onClose: () => void; onSave: (patch: Partial<StoryboardShotPlan>) => void }) {
    ...
    return <Modal open centered footer={null} width={860} closeIcon={<X className="size-5" />} onCancel={onClose} title={`第 ${row[0] || ""} 镜 · 编剧场景卡`}>
        <div className="grid max-h-[72vh] gap-3 overflow-y-auto pr-1">
            {repairError ? <div className="rounded-lg border border-red-300/20 bg-red-500/10 px-3 py-2 text-xs leading-5 text-red-100"><span className="font-semibold">最近修正失败 · {storyboardShotRepairStageLabel(repairError.stage)}：</span>{repairError.message}</div> : null}
...
function PromptStepToolbar({ node, actionKey, promptProgress, dynamicPromptCount, dynamicShotCount, pendingPromptCount, failedPromptCount, staticShotCount, onComposeFinalPrompt, onRecomposeAll, onStopPromptGeneration }: { node: CanvasNodeData; actionKey?: string | null; promptProgress?: { current: number; total: number; phase: string; attempt?: number; status: "running" | "completed" | "paused" | "error" }; dynamicPromptCount: number; dynamicShotCount: number; pendingPromptCount: number; failedPromptCount: number; staticShotCount: number; onComposeFinalPrompt: (node: CanvasNodeData, rowIndex?: number, replaceExisting?: boolean) => void; onRecomposeAll: () => void; onStopPromptGeneration: (node: CanvasNodeData) => void }) {
    ...
    return (
        <div className="flex shrink-0 items-center gap-4">
            ...
            {failedPromptCount ? <div className="text-xs font-semibold text-red-300">失败 {failedPromptCount} · 同一上下文先免费校正，需主动重新合成或上下文变化才请求模型</div> : null}
